...
<title>Check a Stopped Course</title>
</head>
<body class="fixed-top">
	...
	<div class="page-container row-fluid">
		...
		<div class="page-sidebar nav-collapse collapse">
            ...
            <li class="active has-sub"><a href="javascript:;" class=""> <i
				class="icon-list-alt"></i> 课程管理 
				<span class="arrow open"></span>
				<span class="selected"></span> </a>
			<ul class="sub">
				<li><a href="CourseManagementServlet?action=add">添加课程</a>
				</li>
				<li class="active"><a href="CourseManagementServlet?action=stopcourselist">管理教师停课申请</a>
				</li>
		...
		<div class="page-content">
			...
			<div class="container-fluid">
				<div class="row-fluid">
					<div class="span12">
						<h3 class="page-title">审核教师停课申请</h3>
					</div>
				</div>
				<!-- main content start -->
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<%
							//把后台传递过来的数据提取出来，并且显示												
							ArrayList<StopCourseModel> stopCourseList = (ArrayList<StopCourseModel>)request.getAttribute("stopCourseList");																																																																																													
						if(stopCourseList.isEmpty()){
						%>
						<div class="alert">没有申请</div>
						<%
							}
                        else{
                         //有管理员信息，则显示这些信息
						...
						<table class="table-bordered table table-advance table-striped table-hover">
							...
							<tbody>
								<%
				//得到一个用于循环显示的迭代器
                         Iterator<StopCourseModel> stopCourseIterator = stopCourseList.iterator();

                             while(stopCourseIterator.hasNext()){
                             //从迭代器中取到Model
                             StopCourseModel stopCourse = stopCourseIterator.next();
                             CourseModel course = CourseBLL.GetModel(stopCourse.getCourseId());
                             ...
                             //把AdminModel中得值显示出来
								...
								<tr>
									<td><%=stopCourse.getCourseId()%></td>
									<td><%=course.getName()%></td>
									<td><%=course.getTeacherId()%></td>
									<td><%=teacher.getName()%></td>

									<td><a class="btn mini purple"
										href="CourseManagementServlet?action=passstopcourse&stopCourseId=<%=stopCourse.getStopCourseId()%>">
										<i class="icon-edit"></i> 通过</a>
									</td>
									<td><a class="btn mini black"
										href="CourseManagementServlet?action=refusestopcourse&stopCourseId=<%=stopCourse.getStopCourseId()%>">
										<i class="icon-trash"></i> 不通过</a>
									</td>
								</tr>
								<%
									}
